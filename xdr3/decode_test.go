...
func (t *testDecoderFromType) DecodeFrom(d *Decoder) (int, error) {
	v, n, err := d.DecodeInt()
	if err != nil {
		return n, err
	}
...
func (t *testNestedType) DecodeFrom(d *Decoder) (int, error) {
...
// TestEnterScope tests the EnterScope method for depth tracking
func TestEnterScope(t *testing.T) {
	data := []byte{0x00, 0x00, 0x00, 0x01}

	t.Run("decrement depth", func(t *testing.T) {
		d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 10})

		// Initial state: currentDepth should equal maxDepth
		if d.MaxDepth() != 10 {
			t.Errorf("MaxDepth() = %d, want 10", d.MaxDepth())
		}

		// Enter scope should succeed and decrement depth
		if err := d.EnterScope(); err != nil {
	...
	t.Run("depth exceeded", func(t *testing.T) {
		d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 2})

		// First two should succeed
		if err := d.EnterScope(); err != nil {
			t.Fatalf("EnterScope 1 failed: %v", err)
		}
		if err := d.EnterScope(); err != nil {
			t.Fatalf("EnterScope 2 failed: %v", err)
		}

		// Third should fail
		err := d.EnterScope()
		if err == nil {
			t.Fatal("expected error when depth exceeded")
		}
...
// TestLeaveScope tests the LeaveScope method for depth tracking
func TestLeaveScope(t *testing.T) {
	data := []byte{0x00, 0x00, 0x00, 0x01}
	d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 5})
	...
	// Should be able to enter again (we freed up one level)
	if err := d.EnterScope(); err != nil {
		t.Fatalf("EnterScope after LeaveScope failed: %v", err)
	}
}

// TestEnterLeaveScopeBalance tests that Enter/Leave are balanced correctly
func TestEnterLeaveScopeBalance(t *testing.T) {
	data := []byte{0x00, 0x00, 0x00, 0x01}
	d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 3})

	// Simulate nested decoding: enter 3 times, leave 3 times
	for i := 0; i < 3; i++ {
		if err := d.EnterScope(); err != nil {
			t.Fatalf("EnterScope %d failed: %v", i+1, err)
		}
	}
	...
	// Should be able to enter 3 more times again
	for i := 0; i < 3; i++ {
		if err := d.EnterScope(); err != nil {
			t.Fatalf("EnterScope after reset %d failed: %v", i+1, err)
		}
	}
}

// testScopedType implements DecoderFrom using EnterScope/LeaveScope
type testScopedType struct {
	Value int32
}

func (t *testScopedType) DecodeFrom(d *Decoder) (int, error) {
	if err := d.EnterScope(); err != nil {
		return 0, err
	}
	defer d.LeaveScope()
...
// TestDecoderFromWithScope tests that DecodeFrom implementations using EnterScope work correctly
func TestDecoderFromWithScope(t *testing.T) {
	data := []byte{0x00, 0x00, 0x00, 0x2A} // 42

	t.Run("decode with scope", func(t *testing.T) {
		d := NewDecoder(data)
		var result testScopedType
		n, err := result.DecodeFrom(d)
		if err != nil {
	...
	t.Run("depth limit with scope", func(t *testing.T) {
		// With MaxDepth=1, a single DecodeFrom should work
		d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 1})
		var result testScopedType
		_, err := result.DecodeFrom(d)
		if err != nil {
			t.Fatalf("DecodeFrom with depth 1 failed: %v", err)
		}

		// Reset and try with MaxDepth=0 (defaults to 200, so will work)
		d.Reset(data)
	})
}

// testNestedScopedType implements DecoderFrom and contains another testScopedType
type testNestedScopedType struct {
	Outer int32
	Inner testScopedType
}

func (t *testNestedScopedType) DecodeFrom(d *Decoder) (int, error) {
	if err := d.EnterScope(); err != nil {
		return 0, err
	}
	defer d.LeaveScope()
	...
	t.Outer, nTmp, err = d.DecodeInt()
	n += nTmp
	if err != nil {
		return n, err
	}

	nTmp, err = t.Inner.DecodeFrom(d)
	n += nTmp
	if err != nil {
		return n, err
	}
...
// TestNestedDecodeFromWithScope tests depth tracking across nested DecodeFrom calls
func TestNestedDecodeFromWithScope(t *testing.T) {
	// Two int32 values: outer=1, inner=2
	data := []byte{
		0x00, 0x00, 0x00, 0x01, // Outer = 1
		0x00, 0x00, 0x00, 0x02, // Inner.Value = 2
	}

	t.Run("nested decode succeeds with sufficient depth", func(t *testing.T) {
		d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 10})
		var result testNestedScopedType
		n, err := result.DecodeFrom(d)
		if err != nil {
			t.Fatalf("DecodeFrom failed: %v", err)
		}
		...
		if result.Outer != 1 {
			t.Errorf("Outer = %d, want 1", result.Outer)
		}
		if result.Inner.Value != 2 {
			t.Errorf("Inner.Value = %d, want 2", result.Inner.Value)
		}
	})

	t.Run("nested decode fails with insufficient depth", func(t *testing.T) {
		// Need depth 2: one for outer, one for inner
		// With MaxDepth=1, inner should fail
		d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 1})
		var result testNestedScopedType
		_, err := result.DecodeFrom(d)
		if err == nil {
	...
	t.Run("nested decode succeeds with exact depth", func(t *testing.T) {
		// With MaxDepth=2, should just barely work
		d := NewDecoderWithOptions(data, DecodeOptions{MaxDepth: 2})
		var result testNestedScopedType
		n, err := result.DecodeFrom(d)
		if err != nil {
			t.Fatalf("DecodeFrom with exact depth failed: %v", err)
		}
		if n != 8 {
			t.Errorf("bytes read = %d, want 8", n)
		}
